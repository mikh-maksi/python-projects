...
def create_bad_block():
    """ Creates an apple to be eaten """
    global BLOCK_BAD
    posx = SEG_SIZE * random.randint(1, (WIDTH-SEG_SIZE) / SEG_SIZE)
    posy = SEG_SIZE * random.randint(1, (HEIGHT-SEG_SIZE) / SEG_SIZE)
    BLOCK_BAD = c.create_oval(posx, posy,
                          posx+SEG_SIZE, posy+SEG_SIZE,
                          fill="brown")
...
def main():
    ...
    if IN_GAME:
        ...
        elif head_coords == c.coords(BLOCK_BAD):
            IN_GAME = False
        # Self-eating
        else:
...
class Segment(object):
    ...
    def __init__(self, x, y):
        self.instance = c.create_oval(x, y, x+SEG_SIZE, y+SEG_SIZE, fill="#00FF00")
